  ...
  addKeyboardShortcuts() {
    return {
      ...
      "Shift-Tab": () => this.editor.commands.command(({ state, tr }) => {
        if(state.selection.empty) {
          const { parent, parentText, parentOffset, lineStart } = findLine(this.name, state.selection.$from);
...
function findLine(blockName: string, selection: ResolvedPos) {
  ...
  while (index > 0 && !text[index - 1].match(/\r|\n/)) {
    index--;
    console.log(index);
  }
  return {
